...
echo H264 lossless mode selected
echo.
echo ------------------
...
:encoding
	...
	echo.
		...
		echo 	[nvenc_h264 lossless mode, %frate% fps]
		%ffmpegpath% -nostats -loglevel error -framerate %frate% -i "%fold%\snp%%05d.pcx" -c:v nvenc_h264 -preset lossless -pix_fmt nv12 "%fold%".mp4
				if exist "%fold%.mp4" (
