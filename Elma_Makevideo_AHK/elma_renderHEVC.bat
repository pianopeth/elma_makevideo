...
:encoding
	...
	echo.
		...
		%ffmpegpath% -nostats -loglevel error -framerate %frate% -i "%fold%\snp%%05d.pcx" -c:v hevc_nvenc -preset lossless -pix_fmt nv12 "%fold%".mp4
				...
				echo Deleting *.pcx
				del %fold%\snp*.pcx
				echo Deleting _extraseconds.log
				del %fold%\_extraseconds.log
				echo Deleting folder %fold%
				rmdir %fold%
				echo.
				)	else (
...
:end
	...
	echo -------------------
	echo.
rem	echo Note: PCX sequences aren't deleted after mp4 conversion, you have to delete them manually.
rem	echo.
	pause
	exit /B
